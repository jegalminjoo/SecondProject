...
public class Calendar4 extends JDialog {
	...
	public Calendar4(String selectedDate) {
		booleanJcb = false;
...
//		LocalDate selectDate = LocalDate.of(Integer.parseInt(selectDate1[0]), Integer.parseInt(selectDate1[1]), Integer.parseInt(selectDate1[2]));
	...
	protected void makeCalendar(LocalDate date) {
		...
		LocalDate depDay = LocalDate.parse(dayday.trim(), DateTimeFormatter.ofPattern("yyyy / M / d"));
		...
		lblLastMonth.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (lastMonth.getYear() != depDay.getYear() || lastMonth.getMonthValue() >= depDay.getMonthValue()) {
		...
		for (int i = 1; i <= date.lengthOfMonth(); i++) {
			...
			// 이번달인거는 지난날짜 선택불가능
			if(date.getYear()==depDay.getYear() && date.getMonthValue()==depDay.getMonthValue()) {
		    ...
		    // 이번달아닌거는 모든날따 선택가능
			} else if(date.getYear()!=depDay.getYear() || date.getMonthValue()!=depDay.getMonthValue()) {
				JLabel lblday = new JLabel(String.valueOf(i));
				lblday.setHorizontalAlignment(JLabel.CENTER);
				lblday.setPreferredSize(new Dimension(30, 30));
				pnlDay.add(lblday);
