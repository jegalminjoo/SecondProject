...
public class Calendar2 {
	...
	static JPanel pnlReInfor;

	public Calendar2(JPanel pnlReCal, JPanel pnlRe) {
		booleanJcb = false;
		LocalDate date = LocalDate.now();
		makeCalendar(pnlReCal, pnlRe, date);
	}

	public static void makeCalendar(JPanel pnlReCal, JPanel pnlRe, LocalDate date) {
		pnlReCal.removeAll();
		pnlRe.removeAll();
		...
		lblLastMonth.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				pnlRe.removeAll();
				makeCalendar(pnlReCal, pnlRe, lastMonth);
			}
		});
		...
		lblNextMonth.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				pnlRe.removeAll();
				makeCalendar(pnlReCal, pnlRe, nextMonth);
			}
				...
				pnlPassday.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						...
						for (Plane pp : Main.planes) {
							for (int j = 0; j < pp.schedules.size(); j++) {
								if (pp.schedules.get(j).getDeptDay().trim().equals(selectDay.trim())) {
								pnlReInfor = new JPanel();
								pnlReInfor.setLayout(new BoxLayout(pnlReInfor, BoxLayout.Y_AXIS));
								pnlReInfor.setPreferredSize(new Dimension(760, 47));
								pnlReInfor.setMaximumSize(new Dimension(760, 47));
								JLabel lblRe = new JLabel();
								lblRe.setBorder(BorderFactory.createTitledBorder(new LineBorder(new Color(140,140,140), 1)));
								lblRe.setAlignmentX(0.5F);
								lblRe.setHorizontalAlignment(JLabel.CENTER);
								...
								lblRe.setText(" 비행기 편명 : " +pp.planeName +
										...
										+ "   /   예약한 고객 등급(D,G,S)");
								pnlReInfor.add(lblRe);
								pnlRe.add(pnlReInfor);
								}
							}
						}
						pnlRe.revalidate();
